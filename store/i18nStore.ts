...
type I18nStore = {
  ...
  hydrateFromProfile: (preferredLanguage: string | null | undefined) => Promise<void>;
  t: (key: string) => string;
};
...
const translations: Record<Language, Record<string, string>> = {
  en: {
    ...
    // Chat
    chatAutoReplyStub: "Thanks for reaching out — we’ve received your message. A team member will reply shortly.",
  },
  ar: {
    // General
  ...
  de: {
    ...
    // Chat
    chatAutoReplyStub: 'Danke für Ihre Nachricht — wir haben sie erhalten. Ein Teammitglied antwortet in Kürze.',
  },
};
...
export const useI18nStore = create<I18nStore>((set, get) => ({
  ...
  setLanguage: async (lang: Language) => {
    ...
    try {
      ...
      console.error('[i18nStore] AsyncStorage set user-language failed', e);
    }

    if (I18nManager.isRTL !== isRTL) {
      console.log('[i18nStore] RTL change detected', { from: I18nManager.isRTL, to: isRTL });
    }

    set({ language: lang, isRTL });
  },
  hydrateFromProfile: async (preferredLanguage: string | null | undefined) => {
    const normalized = normalizeLanguage(preferredLanguage);
    if (!normalized) return;
    await get().setLanguage(normalized);
  },
  t: (key: string) => {
...
export const initI18n = async () => {
  try {
    ...
    const normalized = normalizeLanguage(storedLang);
    if (normalized) {
      await useI18nStore.getState().setLanguage(normalized);
    }
  } catch (e) {
    console.error('[i18nStore] Failed to load language', e);
  }
};
