...
type SendMode = 'public_auth' | 'private_user' | 'admin';
...
Deno.serve(async (req: Request) => {
  ...
  // Server-side cooldown (best-effort, in-memory)
  const now = Date.now();
  ...
  if (mode !== 'public_auth' && mode !== 'private_user' && mode !== 'admin') {
  ...
  if (mode === 'public_auth' && conversation.type !== 'public') {
  ...
  const senderId: string | null = mode === 'admin' ? userId : userId;
